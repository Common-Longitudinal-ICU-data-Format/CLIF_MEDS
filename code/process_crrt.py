"""CRRT domain processor for continuous renal replacement therapy."""
...
def process_crrt(config: dict, domain_config: dict, data_dir: Path, output_dir: Path):
    """CRRT processor: load crrt_therapy, emit MEDS parquet."""
    ...
    rows = []
    for row_dict in tqdm(df.iter_rows(named=True), total=len(df), desc="  CRRT"):
        ...
        for concept_name, concept_mapping in concepts.items():
            ...
            # Skip events where both numeric_value and text_value are null
