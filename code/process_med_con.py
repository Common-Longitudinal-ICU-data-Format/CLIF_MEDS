"""MED_CON domain processor with clifpy unit standardization."""
...
def process_med_con(config: dict, domain_config: dict, data_dir: Path, output_dir: Path):
    """MED_CON processor: load continuous meds, convert units via clifpy, emit MEDS parquet."""
    filetype = config["filetype"]
    site_tz = config.get("timezone")
    subject_id_col = domain_config.get("subject_id_col", config.get("subject_id_col", "patient_id"))

    # --- Identify the concept section (same pattern as _process_domain) ---
    ...
    # --- Build MEDS rows ---
    rows = []
    for row_dict in tqdm(df.iter_rows(named=True), total=len(df), desc="  MED_CON"):
